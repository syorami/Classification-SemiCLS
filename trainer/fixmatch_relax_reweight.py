...
class FixMatch_Relax_Reweight(Trainer):
    """ FixMatch_Relax_Reweight """
    ...
    def compute_loss(self,
        ...
        topk_probs, topk_labels = probs_u_w.topk(self.cfg.topk, dim=1)
        extend_feats = torch.cat((features, features.repeat(self.cfg.topk - 1, 1, 1)))
        extend_probs = torch.cat((topk_probs[:, 0], topk_probs[:, 1:].permute(1, 0).reshape(-1)))
        extend_labels = torch.cat((topk_labels[:, 0], topk_labels[:, 1:].permute(1, 0).reshape(-1)))
        # In case of early training stage, pseudo labels have low scores
        if labels.shape[0] != 0:
            Lcontrast = self.loss_contrast(extend_feats, extend_probs, labels=extend_labels)
        else:
